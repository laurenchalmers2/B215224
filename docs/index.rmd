...
population <- read_csv(here("data", "healthboardpopulationbyage.csv"), skip = 10) %>% 
  clean_names()  
```
Joining months data form each year to produce whole year datasets 
```{r}
...
```
Selecting and grouping columns to be examined 
```{r}
data_2024 <- data_2024 %>% 
...
Graphing ab types per year- measure most common tyoes rather than per person amounts *need to ask why this isnt printing
```{r}
library(ggplot2)
library(dplyr)
...
all_ab_data_pop %>% 
  ...
  guides(fill = guide_legend(nrow = 1))
```
Mapping some of the results 
```{r}
library(purrr)
library(sf)

class(all_ab_data_pop)

# all_ab_data_pop should be an sf dataframe with geometry and attributes

ggplot(data = all_ab_data_pop) +
  geom_sf(aes(fill = `Antibiotics Per Person`)) +
...
all_ab_data_pop %>%
  select(Year, `Health Board`, `Antibiotic Type`, `Prescription Number`, `Antibiotics Per Person` ) %>%
  gt() %>%
   cols_label(`Health Board` = "Health Board", Year = "Year", `Prescription Number` = "Prescription Number", `Antibiotic Type` = "Antibiotic Type") %>% 
   cols_align(align = "left",
              columns = Year)
    tab_header(title = "Antibiotic Prescriptions Before, During and After Covid-19",
    subtitle = "Data from scottish health boards 2016, 2020, 2024") %>% 
    fmt_number(columns = vars(`Prescription Number`), use_seps = TRUE) %>% 
      summary_rows(columns = c(`Antibiotics Per Person`, `Prescription Number`), 
               fns = list(Average = ~mean(., na.rm = TRUE))) %>% 
  grand_summary_rows(columns = c(`Antibiotics Per Person`, `Prescription Number`), 
...
all_ab_data_pop %>%
  ...
  cols_label(
    ...
    `Antibiotics Per Person` = "Antibiotics per Person"
  ) %>%
  cols_align(
    align = "left",
    columns = vars(Year)
