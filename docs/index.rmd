 ...
 *try and order bars from increasing to decreasing*
...
```{r}
#option making my own colour pallette to match the other grapohs that has more than 8 layers for all different health boards library(RColorBrewer)
...
#loading package needed for creating a table
library(gt)
# table of all data examined
all_prescription_and_hb_data %>%
  select(year, hb_name, antibiotic_type, prescription_number, health_board_population, antibiotics_per_person) %>%  # Move select before gt()
  gt(groupname_col = "year") %>%  # Group by year instead
  cols_label(
  ...
  tab_style(
    style = cell_fill(color = "lightblue"),#could add my own color palette in here
    locations = cells_row_groups()
  ) %>%
  data_color(
    columns = antibiotic_type, 
    method = "factor", 
    palette = "Set2"
  ) %>%  
  summary_rows(
    groups = TRUE,
    columns = c(antibiotics_per_person), 
...
# table of all data examined
all_prescription_and_hb_data %>%
  arrange(year, antibiotic_type, hb_name) %>%  # Sort by year, then antibiotic type
  select(year, antibiotic_type, hb_name, prescription_number, health_board_population, antibiotics_per_person) %>%
  gt(groupname_col = "year", rowname_col = "antibiotic_type") %>%  # Group by year and antibiotic type
  cols_label(
    hb_name = "Health Board", 
    prescription_number = "Prescription Number", 
    health_board_population = "Health Board Population",
...
references <- tibble(
  References = c(
    ...
    "Salcolm, W., Seaton, R.A., Haddock, G., Baxter, L., Thirlwell, S., Russell, P., Cooper, L., Thomson, A. and Sneddon, J., 2020. Impact of the COVID-19 pandemic on community antibiotic prescribing in Scotland. JAC-Antimicrobial Resistance, 2(4), p.dlaa105."
  )
)
...
references %>%
  ...
  tab_options(
    table.width = pct(100),
    table.border.top.color = "lightblue",
    table.border.bottom.color = "lightblue"
  )
```
